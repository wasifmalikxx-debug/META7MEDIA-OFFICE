...
export default async function ReviewBonusPage({ searchParams }: { searchParams: Promise<{ month?: string; year?: string; team?: string }> }) {
  ...
  // Izaan (EM-4) runs his own shops as of May 19 2026. His personal
  // submit form lives on /my-reviews now (separate sidebar entry); on
  // this page he's strictly an approver. We still drop his own pending
  // claims from his team inbox so they only render on the page where
  // he can actually edit/track them — CEO sees + approves them via
  // the SUPER_ADMIN branch below.
  const isIzaan = role === "MANAGER" && employeeId === "EM-4";
  ...
  if (isReviewer) {
    ...
    const submissions = await prisma.reviewBonus.findMany({
      where: {
        ...
        // Izaan's own pending claims never show in his approval inbox —
        // self-approval is blocked at the API anyway, and his personal
        // submissions live on /my-reviews where he can actually edit them.
        // CEO sees + approves them via the SUPER_ADMIN branch (no NOT
        // filter applied to that role).
        ...(isIzaan ? { NOT: { userId } } : {}),
      },
      include: {
        ...
        <PageHeader
          ...
          description={
            role === "PARTNER"
              ...
              : isIzaan
              ? "Approve or reject your team's review bonus submissions. Your own shop claims live on the Submit Review page."
